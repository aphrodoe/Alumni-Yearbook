    ...
    >
      ...
      <div className="space-y-3">
        {poll.options.map((option) => (
          ...
          >
            {voted ? (
              <motion.div className="relative border rounded-md overflow-hidden py-2 px-2 min-h-[3rem]">
                <motion.div
                  className={`absolute top-0 left-0 bottom-0 ${
                    selectedOption === option.id ? 'bg-blue-600' : 'bg-blue-100'
                  }`}
                  initial={{ width: 0 }}
                ...
                />
                <div
                  className="relative z-10 flex flex-wrap items-start justify-between gap-2 w-full px-2 cursor-pointer"
                  onClick={() => handleVote(option.id)}
                >
                  <span className="flex flex-wrap items-center text-black">
                    {getOptionEmoji(option.text)}{' '}
                    <span className="ml-2 break-words">{option.text}</span>
              ...
              <motion.button
                onClick={() => handleVote(option.id)}
                className="w-full text-left p-3 border border-gray-300 rounded-md hover:bg-blue-50 transition flex flex-wrap items-start justify-between group relative overflow-hidden"
                whileTap={{ scale: 0.98 }}
              >
                <span className="flex flex-wrap items-center text-left break-words">
                  {getOptionEmoji(option.text)}{' '}
